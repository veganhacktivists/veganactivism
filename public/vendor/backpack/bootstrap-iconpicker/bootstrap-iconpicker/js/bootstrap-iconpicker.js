...
;(function($) {
  ...
  Iconpicker.prototype.updateIcons = function(page) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      for (var j = 0; j < op.cols; j++) {
        ...
        var btn = $(
          '<button class="btn ' + op.unselectedClass + ' btn-icon"></button>',
        ).hide()
  ...
  Iconpicker.prototype.updateLabels = function(page) {
    ...
    op.table
      ...
      .html(
        op.labelHeader
          .replace('{0}', total_pages === 0 ? 0 : page)
    ...
    op.table.find('.icons-count').html(
      op.labelFooter
        ...
        .replace('{1}', total < total_icons ? total : total_icons)
        ...
        if (i === 0 || i === op.cols - 1) {
          var arrow = [
            '<button class="btn btn-arrow ' +
              ...
              '">',
            '<span class="' +
              (i === 0 ? op.arrowPrevIconClass : op.arrowNextIconClass) +
              '"></span>',
            '</button>',
          ]
  ...
  Iconpicker.prototype.updatePicker = function() {
    ...
    if (op.cols < 4) {
      ...
      throw 'Iconpicker => The number of rows must be greater than or equal to 0. [option.rows = ' +
        op.rows +
        ']'
    } else {
