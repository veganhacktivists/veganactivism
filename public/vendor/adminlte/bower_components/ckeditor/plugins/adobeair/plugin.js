          ...
          for (var b = 0; b < l.length; b++)
            (function(b) {
              ...
              a.hasAttribute('on' + b) &&
                ...
                a.on(b, function(b) {
                  var c = /(return\s*)?CKEDITOR\.tools\.callFunction\(([^)]+)\)/.exec(
                      d,
                    ),
                    k = c && c[1],
                    e = c && c[2].split(','),
      ...
      CKEDITOR.env.air &&
        ((CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(
          ...
          function(a) {
            function c(b, a, c, k) {
              a = b.append(a)
              ;(c = CKEDITOR.htmlParser.fragment.fromHtml(c).children[0]
            ...
            return function(b) {
              if (this.getBody()) {
                var d = this,
                  f = this.getHead()
                b = b.replace(/(<style[^>]*>)([\s\S]*?)<\/style>/gi, function(
                  a,
                  b,
                  d,
                ) {
                ...
                b.replace(/(<body[^>]*>)([\s\S]*)(?=$|<\/body>)/i, function(
                  b,
                  a,
                  c,
                ) {
                  d.getBody().setHtml(c)
                  ;(b = CKEDITOR.htmlParser.fragment.fromHtml(a).children[0]
