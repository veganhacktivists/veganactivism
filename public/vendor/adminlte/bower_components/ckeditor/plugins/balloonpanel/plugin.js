...
;(function() {
  ...
  CKEDITOR.ui.balloonPanel = function(a, b) {
    ...
    this.templates = {}
    for (var d in this.templateDefinitions)
      this.templates[d] = new CKEDITOR.template(this.templateDefinitions[d])
    this.parts = {}
    this.focusables = {}
    this.showListeners = {}
  ...
  CKEDITOR.ui.balloonPanel.prototype = {
    ...
    attach: (function() {
      function a(a) {
        var c = a.getClientRect(),
          k = b.getScrollPosition()
        r || a.equals(d)
          ? ((c.top += k.y), (c.left += k.x))
          : ((a = d.getClientRect()),
            (c.top = a.top + c.top + k.y),
            (c.left = a.left + c.left + k.x))
      ...
      return function(w, c) {
        if (c instanceof CKEDITOR.dom.element || !c) c = { focusElement: c }
        c = CKEDITOR.tools.extend(c, { show: !0 })
        !0 === c.show && this.show()
        this.fire('attach')
        b = CKEDITOR.document.getWindow()
        d = this.editor.window.getFrame()
        ...
        !1 !== c.focusElement && (c.focusElement || this.parts.panel).focus()
      }
    })(),
    resize: function(a, b) {
      this.rect.width = a
    ...
    setTriangle: function(a, b) {
      var d = this.parts.triangleOuter,
        p = this.parts.triangleInner
      this.triangleSide &&
        (d.removeClass('cke_balloon_triangle_' + this.triangleSide),
        d.removeClass('cke_balloon_triangle_align_' + this.triangleAlign),
        p.removeClass('cke_balloon_triangle_' + this.triangleSide))
      this.triangleSide = a
      this.triangleAlign = b
      d.addClass('cke_balloon_triangle_' + a)
      d.addClass('cke_balloon_triangle_align_' + b)
    ...
    _getAlignments: function(a, b, d) {
      return {
        ...
        'top hcenter': {
          top: a.top - d - this.triangleHeight,
