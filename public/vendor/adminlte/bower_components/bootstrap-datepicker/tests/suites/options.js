module('Options', {
  ...
  teardown: function() {
    $('#qunit-fixture *').each(function() {
      var t = $(this)
      if ('datepicker' in t.data()) t.datepicker('remove')
    })
  },
})
...
test('Startview: year view (integer)', function() {
  ...
  ok(
    picker.find('.datepicker-decades').is(':not(:visible)'),
    'Decades view hidden',
  )
  ok(
    picker.find('.datepicker-centuries').is(':not(:visible)'),
    'Centuries view hidden',
  )
})
...
test('Startview: decade view (integer)', function() {
  ...
  ok(picker.find('.datepicker-days').is(':not(:visible)'), 'Days view hidden')
  ok(
    picker.find('.datepicker-months').is(':not(:visible)'),
    'Months view hidden',
  )
...
test('Startview: millennium view (integer)', function() {
  ...
  ok(
    picker.find('.datepicker-centuries').is(':visible'),
    'Centuries view visible',
  )
})
...
test('Today Button: today button not default', function() {
  ...
  ok(
    picker.find('.datepicker-days tfoot .today').is(':not(:visible)'),
...
test('Today Button: today visibility when enabled', function() {
  ...
  ok(picker.find('.datepicker-days').is(':visible'), 'Days view visible')
  ok(
    picker.find('.datepicker-days tfoot .today').is(':visible'),
  ...
  ok(picker.find('.datepicker-months').is(':visible'), 'Months view visible')
  ok(
    picker.find('.datepicker-months tfoot .today').is(':visible'),
    'Today button visible',
  )
  ...
  ok(picker.find('.datepicker-years').is(':visible'), 'Years view visible')
  ok(
    picker.find('.datepicker-years tfoot .today').is(':visible'),
    'Today button visible',
  )
})
...
test(
  "Today Highlight: today's date is not highlighted by default",
  patch_date(function(Date) {
    ...
    ok(picker.find('.datepicker-days').is(':visible'), 'Days view visible')
    equal(
      picker.find('.datepicker-days thead .datepicker-switch').text(),
      'March 2012',
      'Title is "March 2012"',
    )
...
test(
  "Today Highlight: today's date is highlighted when not active",
  patch_date(function(Date) {
...
test('Clear Button: clear visibility when enabled', function() {
  ...
  ok(picker.find('.datepicker-days').is(':visible'), 'Days view visible')
  ok(
    picker.find('.datepicker-days tfoot .clear').is(':visible'),
  ...
  ok(picker.find('.datepicker-months').is(':visible'), 'Months view visible')
  ok(
    picker.find('.datepicker-months tfoot .clear').is(':visible'),
    'Clear button visible',
  )
  ...
  ok(picker.find('.datepicker-years').is(':visible'), 'Years view visible')
  ok(
    picker.find('.datepicker-years tfoot .clear').is(':visible'),
    'Clear button visible',
  )
})
...
test('Active Toggle Multidate Default: when one of the active dates is selected it is unset', function() {
  ...
  target.click()
  ok(
    dp.dates.contains(UTCDate(2012, 2, 4)) === -1,
    '2012-03-04 no longer in dates',
  )
  datesEqual(dp.viewDate, UTCDate(2012, 2, 4))
...
test('DatesDisabled as attribute', function() {
  var input = $(
      '<input data-date-dates-disabled="2012-10-1,2012-10-10,2012-10-20" />',
    )
      .appendTo('#qunit-fixture')
      .val('2012-10-26')
      .datepicker({
...
test('Multidate', function() {
  ...
  // Initial value is selected
  ok(
    dp.dates.contains(UTCDate(2012, 2, 5)) !== -1,
    '2012-03-05 (initial date) in dates',
  )
...
test('Container', function() {
  var testContainer = $('<div class="date-picker-container"/>').appendTo(
      '#qunit-fixture',
    ),
    input = $('<input />')
      .appendTo('#qunit-fixture')
  ...
  equal(
    target.parent()[0],
    testContainer[0],
    'Container is not the testContainer that was specificed',
  )
})
...
test(
  'i18n: Leverage i18n titleFormat when available.',
  patch_date(function(Date) {
    var input = $('<input />')
        .appendTo('#qunit-fixture')
        .val('2015年04月21日')
    ...
    equal(
      picker.find('.datepicker-days thead .datepicker-switch').text(),
      '2015年04月',
      'Title is in Chinese: 2015年04月',
    )
  }),
)

test(
  'i18n: Leverage English (default) i18n titleFormat when translation key for specified language is not available.',
  patch_date(function(Date) {
    var input = $('<input />')
        .appendTo('#qunit-fixture')
        .val('04/21/2015')
    ...
    equal(
      picker.find('.datepicker-days thead .datepicker-switch').text(),
      'April 2015',
      'Title is in default format: April 2015',
    )
  }),
)
...
test('templates', function() {
  ...
  input.focus()
  equal(
    picker.find('.datepicker-days .prev').prop('innerHTML'),
    '<span class="glyphicon glyphicon-arrow-left"></span>',
  )
  equal(
    picker.find('.datepicker-days .next').prop('innerHTML'),
    $('<div>')
      .html('&raquo;')
      .text(),
  )
...
test('updateViewDate', function() {
  ...
  var input = $('<input value="08/03/1990"/>')
      ...
      .on('changeMonth', function() {
        var msg = shouldTriggerChangeMonth
          ? 'changeMonth must be triggered'
          : 'changeMonth must not be triggered'
        ok(shouldTriggerChangeMonth, msg)
      })
      .on('changeYear', function() {
        var msg = shouldTriggerChangeYear
          ? 'changeYear must be triggered'
          : 'changeYear must not be triggered'
  ...
  equal(
    monthShown.text(),
    'May 1945',
    'does not change viewDate on `setDate` method',
  )
  input.focus()
  ...
  equal(
    monthShown.text(),
    'May 1945',
    'does not change viewDate if a day in next month is selected',
  )
  ...
  equal(
    monthShown.text(),
    'January 1946',
    'changing year must still be possible',
  ) // and must trigger `changeYear` and `changeMonth` events
})
