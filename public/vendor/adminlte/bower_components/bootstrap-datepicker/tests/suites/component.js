...
test('Navigating to/from decade view', function() {
  ...
  target.click()
  ok(
    this.picker.find('.datepicker-months').is(':visible'),
  ...
  target.click()
  ok(
    this.picker.find('.datepicker-years').is(':visible'),
...
test(
  ...
  patch_show_hide(function() {
    ...
    var curDivShowing = childDivs.filter(
      '.datepicker-' + viewModes[minViewMode].clsName,
    )
    ok(
    ...
    var divNotShown
    for (
      var curViewMode = minViewMode + 1;
      curViewMode <= maxViewMode;
      curViewMode++
    ) {
      divNotShown = childDivs.filter(
        '.datepicker-' + viewModes[curViewMode].clsName,
      )
      ok(
        divNotShown.hasClass('foo'),
        'Other divs do have overridden `$.fn.hide` side-effects',
      )
    }
  }),
)
