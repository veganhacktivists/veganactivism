    ...
    pixlrLoad = function(mode, base) {
      var self = this,
        ...
        launch = function() {
          ...
          errtm = setTimeout(error, timeout * 1000)
          myurl +=
            (myurl.indexOf('?') === -1 ? '?' : '&') + 'pixlr=' + node.attr('id')
  ...
  return [
            ...
            require.config({
              paths: {
                'fabric/dist/fabric.require':
                  cdns.fabric16 + '/fabric.require.min',
                'tui-code-snippet':
    ...
    },
    {
      ...
      save: function(base) {
        ...
        if (base._canvas) {
          q = $base.data('quty')
            ? Math.max(Math.min($base.data('quty').val(), 100), 1) / 100
            : void 0
      ...
      init: function(id, file, dum, fm) {
        var ta = this,
          ...
          setStatus = function(status) {
            spnr
              .show()
              .children('.elfinder-spinner-text')
              .text(status.info)
          },
